...
// --- LÓGICA DE LA PREGUNTA INTERACTIVA ---

document.addEventListener("DOMContentLoaded", () => {
    const btnNo = document.getElementById("btn-no");

    if (btnNo) {
        // Provoca la evasión del botón al calcular coordenadas arbitrarias de la pantalla
        const esquivarBoton = () => {
            btnNo.style.position = "fixed";
            
            // Limitamos los rangos para que no se salga de los extremos de la ventana
        ...
        // Activadores: pasar el ratón por encima (PC) o pulsar la pantalla (móvil)
        btnNo.addEventListener("mouseenter", esquivarBoton);
        btnNo.addEventListener("touchstart", (e) => {
            e.preventDefault(); // Evita clics no deseados en móviles
            esquivarBoton();
        });
    }
});
...
function celebrateLove() {
    ...
    responseText.innerText = "Siiiiii! yo también te quiero mucho amor 💖";
    responseText.classList.add("show");
    
    // Ocultar el botón NO permanentemente al ganar
    if(btnNo) btnNo.style.display = "none";

    // Disparar oleada secuencial de corazones decorativos
    for (let i = 0; i < 40; i++) {
        setTimeout(() => {
            createHeart();
        }, i * 80);
...
function createHeart() {
    ...
    // Tamaños y velocidades variables para añadir profundidad
    ...
    // Eliminar el elemento del árbol DOM al finalizar su trayectoria
    setTimeout(() => {
        heart.remove();
    }, duration * 1000);
}